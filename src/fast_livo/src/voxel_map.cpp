...
void loadVoxelConfig(const rclcpp::Node::SharedPtr &node_, VoxelMapConfig &voxel_config)
{
  fastlivo_compat::get_param<bool>(node_, "publish/pub_plane_en", voxel_config.is_pub_plane_map_, false);
  
  fastlivo_compat::get_param<int>(node_, "lio/max_layer", voxel_config.max_layer_, 1);
  fastlivo_compat::get_param<double>(node_, "lio/voxel_size", voxel_config.max_voxel_size_, 0.5);
  fastlivo_compat::get_param<double>(node_, "lio/min_eigen_value", voxel_config.planner_threshold_, 0.01);
  fastlivo_compat::get_param<double>(node_, "lio/sigma_num", voxel_config.sigma_num_, 3);
  fastlivo_compat::get_param<double>(node_, "lio/beam_err", voxel_config.beam_err_, 0.02);
  fastlivo_compat::get_param<double>(node_, "lio/dept_err", voxel_config.dept_err_, 0.05);
  {
    // rclcpp integer arrays are std::vector<int64_t>; read as that then narrow.
    std::vector<int64_t> _layer_init_num;
    fastlivo_compat::get_param<std::vector<int64_t>>(node_, "lio/layer_init_num", _layer_init_num,
                                                     std::vector<int64_t>{5, 5, 5, 5, 5});
    voxel_config.layer_init_num_.assign(_layer_init_num.begin(), _layer_init_num.end());
  }
  fastlivo_compat::get_param<int>(node_, "lio/max_points_num", voxel_config.max_points_num_, 50);
  fastlivo_compat::get_param<int>(node_, "lio/max_iterations", voxel_config.max_iterations_, 5);

  fastlivo_compat::get_param<bool>(node_, "local_map/map_sliding_en", voxel_config.map_sliding_en, false);
  fastlivo_compat::get_param<int>(node_, "local_map/half_map_size", voxel_config.half_map_size, 100);
  fastlivo_compat::get_param<double>(node_, "local_map/sliding_thresh", voxel_config.sliding_thresh, 8);
}
