...
{
    node_ = node;
    fastlivo_compat::get_param<std::string>(node_, "laserMapping/outputfilepath", outputfilepath, "output");
    ...
    // Create the output directory tree so file writes don't silently fail when
    // outputfilepath points somewhere that doesn't exist yet (e.g. a plain
    // `ros2 launch`/installed run outside the Docker workspace).
    std::error_code _ec;
    for (const auto &sub : {"/TUM", "/vel", "/global_pcd", "/scan_pcd", "/debug"})
        std::filesystem::create_directories(outputfilepath + sub, _ec);

    fastlivo_compat::get_param<string>(node_, "gps/gps_topic", gps_topic, "/ublox_driver/receiver_lla");
    fastlivo_compat::get_param<double>(node_, "gps/gps_time_offset", gps_offset, 0.0);
    fastlivo_compat::get_param<bool>(node_, "gps/gps_en", gps_en, false);
    fastlivo_compat::get_param<vector<double>>(node_, "gps/extrinsic_T", gps_extrinT, vector<double>());

    fastlivo_compat::get_param<bool>(node_, "opt/debug_mode", debug_mode, false);
    fastlivo_compat::get_param<double>(node_, "opt/rtk_cov", rtk_cov, 0.02);
    fastlivo_compat::get_param<double>(node_, "opt/livo2_RPY_cov", livo2_RPY_cov, 1e-4);
    fastlivo_compat::get_param<double>(node_, "opt/livo2_XYZ_cov", livo2_XYZ_cov, 1e-4);

    pubGpsOdom = node_->create_publisher<nav_msgs::msg::Odometry>("/gps/odometry_opt", rclcpp::QoS(rclcpp::KeepLast(2000)));

    // subGPS = node_->create_subscription<sensor_msgs::msg::NavSatFix>(gps_topic, ...);
    ...
    const auto mf_qos = rclcpp::QoS(rclcpp::KeepLast(10000)).get_rmw_qos_profile();
    subOdom_.subscribe(node_, "/odometry/fast_livo2", mf_qos);
    subCloud_.subscribe(node_, "/synced_cloud", mf_qos);
    // Larger ApproximateTime queue so keyframe pairs are not dropped before sync.
    sync_ = std::make_unique<message_filters::Synchronizer<SyncPolicy>>(SyncPolicy(200), subOdom_, subCloud_);
    sync_->registerCallback(std::bind(&optimization::syncedCallback, this, std::placeholders::_1, std::placeholders::_2));
...
{
    // The offline-optimization worker blocks on std::cin.get() and cannot be
    // cleanly joined; detach it so destroying this object with a still-joinable
    // thread does not call std::terminate() on normal exit / Ctrl-C.
    if (optimization_thread_.joinable()) optimization_thread_.detach();
}
...
void optimization::initialAlign()
{
    ...
    for (const auto& g : gpsQueue) {
        const double t = toSec(g.header.stamp);
    ...
    auto find_nearest_gps = [&](double t_query) -> const nav_msgs::msg::Odometry* {
        if (gpsQueue.empty()) return nullptr;

        auto it_lower = std::lower_bound(gpsQueue.begin(), gpsQueue.end(), t_query,
            [](const nav_msgs::msg::Odometry& msg, double t) {
                return toSec(msg.header.stamp) < t;
            });
        ...
        const nav_msgs::msg::Odometry& after = *it_lower;
        const nav_msgs::msg::Odometry& before = *std::prev(it_lower);

        double dt_after = std::abs(toSec(after.header.stamp) - t_query);
        double dt_before = std::abs(t_query - toSec(before.header.stamp));
    ...
    for (size_t i = 0; i < cloudKeyPoses6D->size(); ++i) 
    {
        ...
        nav_msgs::msg::Odometry gps_odom_B;
        ...
        const nav_msgs::msg::Odometry* nearest_gps = find_nearest_gps(tk);
...
void optimization::syncedCallback(const nav_msgs::msg::Odometry::ConstSharedPtr& odomMsg, const sensor_msgs::msg::PointCloud2::ConstSharedPtr& cloudMsg)
{
    timeLaserInoStamp = cloudMsg->header.stamp;
    timeLaserInfoCur  = toSec(cloudMsg->header.stamp);
...
void optimization::gpsHandler(const gnss_comm::msg::GnssPVTSolnMsg::ConstSharedPtr& gpsMsg)
{
...
void optimization::writeRtkTumTrajectory() {
    ...
    for (const auto& odom_msg : gpsQueue) {
        tum_file << toSec(odom_msg.header.stamp) << " " 
